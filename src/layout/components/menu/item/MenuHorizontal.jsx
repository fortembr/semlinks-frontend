...
export default function MenuHorizontal(props) {
  ...
  const menuItem = navigation.map((item, index) => {
    return item.subMenu ? (
      <SubMenu
        key={item.id}
        title={
          <Row key={item.header} align="middle" className="menu-item hp-px-12 hp-py-4 hp-border-radius">
            <Col>{item.header}</Col>
        ...
        className={splitLocation[1] == item.id && 'ant-menu-submenu-selected'}
      >
                ...
                childItem.children ? (
                  ...
                  >
                    {childItem.id === 'product-detail' ? (
                      <Link to={childItem.navLink} onClick={() => dispatch(loadCurrentItem(products[0]))}>
                        {childItem.title}
                      </Link>
                    ) : childItem.id.split('-')[0] === 'email' ? (
                      <a href={childItem.navLink} target="_blank" rel="noreferrer">
                        {childItem.title}
                      </a>
                    ) : (
                      <Link to={childItem.navLink}>{childItem.title}</Link>
            ...
            <Menu.Item
              ...
              className={
                splitLocation[splitLocation.length - 2] + '/' + splitLocation[splitLocation.length - 1] ===
                  value.navLink &&
                value.navLink.split('/')[value.navLink.split('/').length - 2] + '/' + value.navLink &&
  ...
  return (
    <Menu mode="horizontal" theme={customise.theme == 'light' ? 'light' : 'dark'}>
      {menuItem}
    </Menu>
  );
}
