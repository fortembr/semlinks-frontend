...
export const columns = [
  {
    ...
    render: (dataIndex) => {
      ...
      return (
        <Link onClick={() => store.dispatch(getUser(dataIndex[0]))} to={`/apps/contact/contact-detail/${dataIndex[0]}`}>
          {dataIndex[1] != null ? (
            ...
            imageSplit[imageSplit.length - 1] == 'svg' ? (
              ...
              <Avatar size={48}>
                {dataIndex[1].split(' ')[0][0].toUpperCase() + '' + dataIndex[1].split(' ')[1][0].toUpperCase()}
              </Avatar>
            )
          ) : (
            <Avatar size={48} icon={<User set="curved" className="hp-text-align-center" />} />
          )}
        </Link>
      );
    }
