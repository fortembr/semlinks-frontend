    ...
    <Col span={24}>
      <Card className="hp-border-color-black-40 hp-mb-32 hp-eCommerceCardOne hp-eCommerceCardOne-large">
        <Row gutter={32}>
          ...
          <Col md={14} span={24}>
            {value.rate && <Rate defaultValue={parseFloat(value.rate)} allowHalf></Rate>}
            ...
            <Row align="middle" className="hp-my-8">
              {value.discount ? (
                ...
                <p className="hp-mb-0 hp-text-color-primary-1 hp-font-weight-500">${value.price}</p>
              )}

              {value.discount && (
                <p className="hp-mb-0 hp-text-color-primary-1 hp-font-weight-500">${value.discount}</p>
              )}
            </Row>

            <Row gutter={[8, 8]}>
              {value.detailBtn && (
                <Col xl={10} span={24} className="hp-mt-xs-8">
                  <Link
                    to={`/apps/ecommerce/product-detail/${value.id}`}
                    onClick={() => dispatch(loadCurrentItem(value))}
                  >
                    <Button
                      className="hp-mr-8 hp-text-color-black-60 hp-hover-text-color-primary-1 hp-hover-text-color-dark-primary-2"
                      block
                    >
                      Check Detail
                    </Button>
              ...
              {!value.addToCartCheck ? (
                ...
                <Col xl={14} span={24}>
                  <Row gutter={[8, 8]}>
                    ...
                    <Col xl={7} span={12}>
                      <InputNumber min={1} max={99} value={value.qty} onChange={onChangeHandler} className="hp-w-100" />
                    </Col>
                  </Row>
                </Col>
              )}
