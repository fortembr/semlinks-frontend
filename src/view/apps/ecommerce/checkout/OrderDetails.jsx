      ...
      <Col span={24}>
        <Row gutter={[32, 32]}>
          <Col lg={18} span={24}>
            ...
            <Row gutter={[32, 32]}>
              {cart.length !== 0 &&
                cart.map((value) => <OrderProduct key={value.id} value={value} valueQty={value.qty} />)}
            </Row>
          </Col>

          {cart.length !== 0 ? (
            <Summary totalItem={totalItem} totalPrice={totalPrice} stepUrl="/apps/ecommerce/address-information" />
          ) : (
            <Col span={24}>
              <Empty
                className="hp-mt-32"
