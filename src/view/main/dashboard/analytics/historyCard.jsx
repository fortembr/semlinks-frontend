              ...
              <Row align="middle" justify="space-between">
                <Col sm={12} span={24} className="hp-mb-sm-16">
                  <Row align="middle" wrap={false}>
                    {item.avatar && (
                      <Col>
                        <div className="hp-border-radius-xl hp-cursor-pointer hp-border-1 hp-border-color-dark-80 hp-mr-16">
                          <div
                            className={`hp-border-radius-lg hp-overflow-hidden hp-m-4 hp-d-flex${item.avatarBg}`}
                            style={{ minWidth: 64, width: 64, height: 64 }}
                          >
                    ...
                    <Col>
                      {item.name && <span className="hp-d-block h4">{item.name}</span>}

                      {item.title && <span className="hp-d-block hp-p1-body hp-mt-4">{item.title}</span>}
                    </Col>
                  </Row>
                </Col>

                <Col sm={6} span={12} className="hp-text-sm-left hp-text-right" style={{ minHeight: 50 }}>
                  {item.price && <span className="hp-d-block h4 hp-font-weight-400">{item.price}</span>}

                  {item.number && <span className="hp-d-block hp-p1-body hp-mt-4">{item.number}</span>}
                </Col>

                <Col sm={6} span={12} className="hp-text-right" style={{ minHeight: 50 }}>
                  {item.percent && <span className="h4">{item.percent}</span>}
                </Col>
              </Row>
            </Col>
          ))}
